...
// Remove session validation - this is now a public page
...
if (isset($_POST['signup'])) {
    try {
        ...
        if (!$hasErrors) {
            ...
            // Register student with BLOCKED status (0)
            $hashed_password = password_hash($inputData['password'], PASSWORD_DEFAULT);
            $status = 0; // CHANGED: Block account by default - admin must activate
            ...
            if ($success) {
                ...
                $_SESSION['student_name'] = $inputData['first_name'] . ' ' . $inputData['last_name'];
                $logger->write("New student registration (pending approval): {$inputData['first_name']} {$inputData['last_name']}, ID: {$inputData['studentid']}, Course: {$inputData['course']}, Year: {$inputData['year_level']}");
                $_POST = array();
...
<div class="signup-container">
    ...
    <div class="info-banner">
        ...
        <p>After submitting your registration, your account will be pending approval. An administrator will review and activate your account. You will be able to log in once your account is approved.</p>
    </div>
    ...
    <form name="signup" method="POST" enctype="multipart/form-data" id="signupForm">
        ...
        <div class="form-actions">
            <button type="submit" name="signup" class="btn" id="submitBtn">Submit Registration</button>
